# io_uring variant of engine_api_test.pro (same driver, the pipelined io_uring TransferThread
# linked instead of the async one; needs liburing). See cases/engine_api_uring.py.
...
CONFIG   -= app_bundle
TEMPLATE  = app
TARGET    = engine_api_uring_test
DEFINES  += ULTRACOPIER_PLUGIN_IO_URING _FILE_OFFSET_BITS=64 UNICODE _UNICODE _LARGE_FILE_SOURCE=1
...
HEADERS += $$SPEC/StructEnumDefinition.h $$SPEC/StructEnumDefinition_CopyEngine.h \
    ...
    $$SPEC/pipeline/TransferThreadPipelined.h $$SPEC/uring/TransferThreadUring.h
...
SOURCES += $$PWD/engine_api_test.cpp \
    ...
    $$SPEC/pipeline/TransferThreadPipelined.cpp $$SPEC/uring/TransferThreadUring.cpp
