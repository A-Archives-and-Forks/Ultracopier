...
| `closefail:<substr>`  | `close()` of a matching path really closes the fd but returns `-1`/`EIO` -- a writeback-cached write error (NFS/CIFS/USB) that only surfaces at `close()`. The engine must treat the file as FAILED (error policy), never as copied; a MOVE keeps its source (`cases/close_error_surfaced.py`). ASYNC/libc only (io_uring closes through the ring). | path |
